...
#* Exercise 1: Calculate your age in years as of today

difftime(Sys.Date(), as.Date("1980-06-25"), units = "days") / 365.2425 #* Not my real birthday

difftime(Sys.time(), as.POSIXct("1980-06-25"), units = "days") / 365.2425
...
difftime(Scout$date.first, Scout$date.scout)


#* 2. How long did it take each boy to earn Eagle

difftime(Scoute$date.eagle, Scout$date.scout)
...
Scout %<>%
  mutate(time.to.first = difftime(date.first, date.scout, units = "days"),
         first.year = time.to.first < 365,
         time.to.eagle = difftime(date.eagle, date.scout, units= "days"),
         time.to.eagle = ifelse(is.na(time.to.eagle),
                                difftime(date.grad, date.scout),
                                time.to.eagle),
         eagle = !is.na(date.eagle)) %>% 
  filter(!is.na(time.to.first) & !is.na(date.birth))
